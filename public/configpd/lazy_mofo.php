    ...
    function grid($error = ''){
        ...
        foreach($result as $row){
            ...
            foreach($columns as $index => $column_name){
                ...
                // output
                elseif(array_key_exists($column_name, $this->grid_output_control)) $html .= '    <td>' . $this->get_output_control($column_name . '-' . $row[$this->identity_name], $value, $this->grid_output_control[$column_name], 'grid') . "</td>\n";
...
document.forms[0].submit();
    ...
    function get_input_control($column_name, $value, $command, $called_from){
        ...
        if($cmd == 'text')
            return "<input style='min-width: 100px;width:100%' type='text' name='$column_name' class='$class' value='" . $this->clean_out($value) . "' size='30' />";
        elseif($cmd == 'hidden')
        ...
        elseif($cmd == 'date')
            return "<input type='text' name='$column_name' class='lm_date_select' value='" . $this->date_out($value) . "' size='18' />";
        elseif($cmd == 'datetime')
            return "<input type='text' style='width:150px' name='$column_name' class='lm_datetime_select' value='" . $this->date_out($value, true) . "' size='25' />";
        elseif($cmd == 'textarea')
