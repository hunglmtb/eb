...
if(!$.ui){
	loadjscssfile("/eb/common/js/jquery-ui.js", "js");
	loadjscssfile("/eb/common/css/jquery-ui.css?2", "css");
}
if($.ui){
...
User <span style="cursor:pointer" onclick="location.href='/eb/user/settings.php';"><font color="#33b5e8"><span id="textUsername"><?php echo $current_username; ?></span></font></span> &nbsp;|&nbsp; <div style="display:none;width:50px;cursor:pointer;padding:2px;color:#33b5e8;margin:2px;font-size:8pt">Alert: 0</div>
<a style="color:#33b5e8;text-decoration:none" href="/eb/logout">logout</a> &nbsp;&nbsp;

<img atl="Workflow" src='/eb/img/gear.png' height=16 onclick="showWorkflow()" style="float:right;margin:0px 2px;cursor:pointer">
...
<img atl="Help" src='/eb/img/help.png' height=16 onclick="showHelp()" style="float:right;cursor:pointer">
<script>
...
function showWorkflow(){
	...
	show_wf_loading();
	$("#iframeWorkflow").attr("src","/eb/wf/wfshow.php");
}
var help="";
function showHelp(){
	...
	if(help==""){
		...
		$("#boxHelp").html('<img class="center_content" src="/eb/wf/images/loading.gif">');
		$.get("/eb/common/act.php?act=get_help&func_code="+func_code,function(data){
			help="1";
...
function showTaskLog(task_id){
	...
	$("#boxTaskLog").html('<img class="center_content" src="/eb/wf/images/loading.gif">');
	$.get("/eb/common/act.php?act=get_tasklog&task_id="+task_id,function(data){
...
function _loadTasksCounting(){
	...
	$.get("/eb/taskman/ajaxs/WorkflowTaskCounter.php?user="+username,function(data){
			$("#wf_notify").html(data);
			if(data=="0") 
				$("#wf_notify_box").hide();
			else
...
taskCountingTimer=setTimeout(_loadTasksCounting,30000);
	...
	<div id="boxWorkflow" style="display:none;width:100%;height:100%;background:#ffffff;overflow:hidden;">
		...
		<div id="wf_loading_box" style="position:absolute;left:0px;top:0px;width:100%;height:100%;background:white;opacity:0.8"><center id="notify_splash"><img class="center_content" src="/eb/wf/images/loading.gif"></center></div>
	</div>
<!-- <div id="boxWorkflow" onclick="showWorkflow()" style="z-index:1;display:none;position:fixed;top:0px;left:0px;width:100%;height:100%;background:rgba(0,0,0,0.3)">
	<div id="boxWorkflow_content" style="z-index:100;padding:0px;position:absolute;top: 99px;left:50%;transform: translateX(-50%);width:900px;height:520px;background:#ffffff;border:1px solid #333333;border-radius:0px;box-shadow:0px 5px 20px rgba(0,0,0,0.4)">
		...
		<div id="wf_loading_box" style="position:absolute;left:0px;top:0px;width:100%;height:100%;background:white;opacity:0.5"><center id="notify_splash"><img style="position: absolute;top: 50%;left:50%;transform: translate(-50%,-50%);" src="/eb/wf/images/loading.gif"></center></div>
	</div>
</div> -->
<div id="boxHelp" style="display:none;width:100%;height:100%"><img class="center_content" src="/eb/wf/images/loading.gif"></div>
