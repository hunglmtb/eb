...
use App\Models\FlowCompDayAlloc;
use App\Models\EnergyUnitCompDayAlloc;
use App\Models\WellCompIntervalDayAlloc;
use App\Models\WellCompDayAlloc;
use App\Models\WellComp;
use App\Models\WellCompInterval;
use App\Models\WellCompIntervalPerf;
use App\Models\WellCompIntervalPerfDayAlloc;
use App\Models\AllocRunnerObjects;
use App\Models\FlowDayAlloc;
use App\Models\EnergyUnitDayAlloc;
use App\Models\TankDayAlloc;
use App\Models\StorageDayAlloc;
use App\Models\FlowCoEntDayAlloc;
use App\Models\EnergyUnitCoEntDayAlloc;
...
class Run extends Command {
	 ...
	 * @var string
	 */
	protected $signature = 'runAllo {taskid} {job_id} {alloc_act} {type} {from_date} {to_date} {email}';
	...
	public function handle() {	
		...
		$count = $this->argument ();
		$objProcess = new RunProcess ();
 		if ($count >= 6) {
			$taskid = $this->argument ( 'taskid' );
			$job_id = $this->argument ( 'job_id' );
			$alloc_act = $this->argument ( 'alloc_act' );
			$date_type = $this->argument ( 'type' );
			...
			if ($date_type == "day") {
				$date = date ( 'Y-m-d' );
				$from_date = date ( 'm/d/Y', strtotime ( $date . ' -1 day' ) ) . "";
				$to_date = $from_date;
			}
		} else {			
			$this->_log ( "Invalid task input params", 1 );
			$objProcess->finalizeTask ( $task_id, 3, $log, $email );
		}
		
		$this->fff ();
		
		if (! ($job_id > 0) && isset($runner_id) && ! ($runner_id > 0)) {
		...
		if ($job_id) {
			...
			if ($daybyday == 1) {
				...
				$d2 = date('Y-m-d', strtotime($to_date));
				
				while ( strtotime ( $d1 ) <= strtotime ( $d2 ) ) {
					$ds = explode ( "-", $d1 );
					$dd = "$ds[1]/$ds[2]/$ds[0]";
					// _log("ppppppppppppp: $dd",1);
		...
		$this->_log("Finish. (no error)",2);
		
		if(isset($task_id) >0){
			$objProcess->finalizeTask($task_id,($error_count>0?3:1),$log,$email);
			if($log){
				$data = array('name' => $log);
				$subject = "Automatic allocation task's log"; //($error_count>0?"[ERROR] ":"")."Automatic allocation task's log";
				
				 $data = array('name' => "");
				 $subject = "Automatic allocation task's log"; //($error_count>0?"[ERROR] ":"")."Automatic allocation task's log";
				
				 $ret = Mail::send('(front.sendmail,["log"=>'.$log.'])',$data, function ($message) {
				 	
				 $message->from('manhnv3434@gmail.com', 'Your Application');
				
				 $message->to($email)->subject("Automatic allocation task's log");
	...
	function _log($s,$type)
	{
		global $error_count,$log,$task_id;
		$ret=true;
		//$h=$s;
		if($type==1){
	...
	public function run_runner($runner_id, $from_date, $to_date, $alloc_attr, $alloc_phase, $theor_phase, $alloc_comp, $alloc_type, $theor_attr)
	{
		global $error_count;
	...
	public function exec_runner($runner_id, $from_date, $to_date)
	{
		global $error_count;
	...
	public function exec_job($job_id, $from_date, $to_date)
	{
