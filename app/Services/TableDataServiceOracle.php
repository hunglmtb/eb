...
class TableDataServiceOracle{
    ...
    public $modelTable = null;             // required, column name of id primary key
    
    public $rename = array();               // optional, associative array of column names and friendly names. example: array('prod_id' => 'Product ID') 
    public $required = array();               // optional, associative array of column names and friendly names. example: array('prod_id' => 'Product ID') 
    ...
    public $grid_limit = 50;                  // pagination limit number of records per page
    ...
    public $form_saveas_button = "<input type='button' value='Save as new record' class='lm_button' onclick='_saveAs();'/>"; 
    public $form_back_button   = "<input type='button' value='&lt Back/Cancel' class='lm_button' onclick='_back();'; />"; // use type=button for delete and cancel so form presses the right button with enter key
    ...
    public $grid_delete_link = "<a href='#' onclick='return _delete([identity_id]);'>[delete]</a> <a href='#' onclick='return _genSQL([identity_id]);'>[sql]</a>";
    ...
    public $grid_text_delete = "Select";
    ...
    */
    function __construct($dbh){
        ...
        if(!isset($_SESSION['_csrf']))
            $_SESSION['_csrf'] = '';
		$this->db_schema = strtoupper(config('database.connections')[config('database.default')]['username']);
    }
    ...
    function setModelName($tablename){
    	$this->table 		= $tablename;
    	$this->modelTable 	= \Helper::getModelName ( $tablename);
    }
    ...
    function insert(){
        ...
        if($this->return_to_edit_after_insert)
            $action = 'action=edit&amp;';
        // redirect user
        $url = $this->get_uri_path() . "{$action}_success=1&amp;$this->identity_name=$id&amp;table=$this->table";
    ...
    function sql_delete(){
        ...
        $sql = "delete from $this->table where $this->identity_name = :identity_id";
    ...
    function sql_update(){
        ...
        foreach($_POST as $key => $val){
            ...
            if(!(array_search($key, $columns) === false)){
                $sql_set .= "$key = ?, "; 
                $sql_param[] = $this->cast_value($val, $key);
            }
        }
        $sql_set = rtrim($sql_set, ', ');
        // run sql update
        if($sql_set != ''){
            $sql_final = "update $this->table set $sql_set where $this->identity_name = ?";
			$sql_param[] = $identity_id;
    ...
    function sql_update_grid($context = 'grid'){
        ...
        if(!is_array($arr_delete))
            $arr_delete = array();
/*==== DISABLED DELETE FUNCTION WHEN SAVING DATA ==============
        // delete records
        $sql = '';
        $sql_param = array();
...
//echo "identity_name:$this->identity_name / get:".$_GET[$this->identity_name];
        // run query
...
if($identity_id<=0)
		$identity_id=$_GET[$this->identity_name];
//echo "a:".$_GET[$this->identity_name];
        ...
        $html .= "<table cellpadding='2' cellspacing='1' border='0' width='100%' class='lm_form'>\n";
        ...
        foreach($columns as $column_name){
            ...
            $html .= "<tr>\n";
            $html .= "    <td class='lm_label'>".($this->required[$column_name]?"<span style='color:red'>(*) </span>":"")."$title</td>\n";
            $html .= "    <td>$control</td>\n";
            $html .= "</tr>\n";
        }
        $html .= "</table>\n";
        ...
        // add buttons
        if($action == 'edit')
            $html .= "<div class='lm_form_button_bar'>$this->form_back_button $this->form_delete_button $this->form_update_button $this->form_saveas_button</div>";
        else
    ...
    function grid($error = ''){
        ...
        if(mb_strlen($sql) == 0)
            $sql = "select * from $this->table order by $default_order_by";
        ...
        if($_pagination_off == 0 && $_export == 0)
			$sql = "select * from (select a.*, ROWNUM rrrrnum from ($sql) a where rownum <= ".($grid_limit+$_offset-1).") where rrrrnum >= $_offset";
        // run query
        ...
        $grid_add_link = str_replace('[tablename]', "&name=".$this->table, $grid_add_link);
        ...
        $grid_edit_link = str_replace('[tablename]', "&name=".$this->table, $grid_edit_link);
        ...
        if(count($this->grid_input_control) > 0 || $this->grid_multi_delete == true)
            $button = "<input type='submit' style='display:none' id='button_save_changes' name='__update_grid' value='$this->grid_text_save_changes' class='lm_save_changes_button' />";
        ...
        if($this->grid_show_search_box)
            $search_box = "<form action='$uri_path"."table=$this->table"."' method='post' class='lm_search_box'><input type='text' name='_search' value='" . $this->clean_out($_search) . "' size='20' class='lm_search_input' /> <input type='submit' value='$this->grid_text_search' class='lm_search_button' /><input type='hidden' name='action' value='search'  /><input type='hidden' name='_csrf' value='$_SESSION[_csrf]' />".$html .\Form::token()."</form>";
        $add_record_search_bar = "<table style='display:none' cellpadding='2' cellspacing='1' border='0' width='100%'><tr><td style='text-align: left'>$grid_add_link $grid_export_link</td><td style='text-align: right'>$search_box</td></tr></table>\n";
        ...
        $html .= "<table id='lm_grid' cellpadding='2' cellspacing='1' border='0' class='lm_grid'>\n";
        $html .= $head;
        // print rows
        $j = 0;
        foreach($result as $row){
            ...
            foreach($columns as $index => $column_name){
                $value 				= $row[$column_name];
                $idValue			= $row[$this->identity_name];
                
                // input fields
                if(array_key_exists($column_name, $this->grid_input_control)){
                	$input_control 	= $this->grid_input_control;
                    $mdl 			= $this->modelTable;
                    $command 		= $input_control[$column_name];
                    if ($mdl&&method_exists($mdl,"getForeignColumn")){
                ...
                // edit & delete links
                if($i == ($column_count - 1))
                    $html .= "    <td nowrap='nowrap'>" . str_replace('[identity_id]', $row["id"], $links) . "</td>\n";
                $i++; // column index
            }
            $html .= "</tr>\n";
            // repeat header
...
document.forms[0].elements['action'].value='insert';
...
return true;
    ...
    function html_select($field_name, $value, $sql, $js_or_css = '', $first_option_blank = true, $multiple = 0){
        ...
        $prev_sql = $sql;
        $valueText = "<b style='display: none'>".$this->clean_out($value)."</b>";
        ...
        if($multiple == 0){
            ...
            foreach($result as $row){
                ...
                if($val == $value){
                    ...
                    $valueText = "<b style='display: none'>".$this->clean_out($val)."</b>";
                }
                $out .= "<option value='" . $this->clean_out($val) . "' $selected>" . $this->clean_out($opt?$opt:$val) . "</option>";
            }
            $out = "<select id='$field_name' name='$field_name' class='$class' $js_or_css>$out</select>";
            return $out;
        }
        ...
        foreach($result as $row){
            ...
            if(mb_strstr($value, "$this->delim$val$this->delim")){
                $selected = "selected='selected'";
                $valueText = "<b style='display: none'>".$this->clean_out($opt)."</b>";
            }
        
            $out .= "<option value='" . $this->clean_out($val) . "' $selected>" . $this->clean_out($opt) . "</option>";
        }    
        $out = "<select id='{$field_name}[]' name='{$field_name}[]' class='$class' multiple='multiple' size='$multiple' $js_or_css>$out</select>";
        return $out;
    }
    ...
    function format_title($field_name, $friendly_name = ''){
		...
		//if(strpos(strtolower($field_name),"_id"))
		//	$field_name=substr($field_name,0,strlen($field_name)-3);
    ...
    function get_input_control($column_name, $value, $command, $called_from){
        ...
        elseif($cmd == 'number')
            return "<input type='text' name='$column_name' class='$class' value='" . $this->clean_out($value) . "' size='18' /><b style='display: none'>$this->clean_out($value)</b>";
        elseif($cmd == 'date')
            return "<input type='text' name='$column_name' class='lm_date_select' value='" . $this->date_out($value) . "' size='18' /><b style='display: none'>$this->date_out($value)</b>";
        elseif($cmd == 'datetime')
            return "<input type='text' style='width:150px' name='$column_name' class='lm_datetime_select' value='" . $this->date_out($value, true) . "' size='25' /><b style='display: none'>$this->date_out($value)</b>";
        elseif($cmd == 'textarea')
        ...
        elseif(startsWith($cmd,'select'))
		{
            return $this->html_select($column_name, $value, $command, '', (endsWith($cmd,'#')?false:$this->select_first_option_blank), 0);
		}
    ...
    function get_columns($context = ''){
        ...
        while($column = $sth->getColumnMeta($i++)){
			if(!$column['name'] || $i>100)
				break;
            array_push($columns, strtolower($column['name']));
		}
        ...
        $sql = "select column_name, DATA_DEFAULT column_default from user_tab_cols where DATA_DEFAULT is not null and table_name = ?";
        $sql_param = array($table);
        $result = $this->query($sql, $sql_param, 'get_columns() - populate form_default_value');
		//\Log::info($result);die("qqqq");
        foreach($result as $row){
            if($context == 'form'){
