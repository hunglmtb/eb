...
class autoSaveFlow extends Job implements ShouldQueue, SelfHandling
{
...
//      	\DB::enableQueryLog();
		...
		if($from_date && $to_date){
			...
			while (strtotime($d1) <= strtotime($d2)) {
				$occur_date=$d1;
				saveData($occur_date,$facility_id,$record_freq,$flow_phase,$objectIds);
			...
			if($this->log){
				if (filter_var($email, FILTER_VALIDATE_EMAIL)) {
					try
					{
						...
						$subjectName = ($this->error_count>0?"[ERROR] ":"")."Automatic save Flow task's log";
